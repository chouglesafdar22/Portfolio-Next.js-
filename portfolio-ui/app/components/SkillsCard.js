    ...
    return (
        <div
            ...
            <div className='flex justify-between items-center'>
                <h3 className='lg:text-4xl sm:text-3xl text-2xl font-normal'>{title}</h3>
                <p className='lg:text-3xl sm:text-2xl text-xl text-center font-normal'>{icon ? <IoIosArrowDropup /> : <IoIosArrowDropdown />}</p>
            </div>
            <div className={`${icon ? "block" : "hidden"} lg:text-6xl sm:text-5xl text-4xl font-light flex flex-wrap md:gap-5 gap-3.5 justify-items-start`}>
                <p>{icon1}</p>
                <p>{icon2}</p>
                <p>{icon3}</p>
                <p>{icon4}</p>
